...
管理WorkerのCronは毎日03:00（日本時間、UTCでは `0 18 * * *`）に実行され、次を行います。

- 90日を過ぎた匿名アクセス解析セッションを日別・タイプ別の集計表へ加算
- 集計済みの90日を過ぎた匿名アクセス解析セッションを削除
- 有効期限を過ぎた管理画面ログインセッション

詳細データの保存期間は90日、圧縮した日別集計は無期限です。日別集計には個々の匿名IDを残さず、訪問・回答・診断完了・noteクリック・タイプ別件数などの数値だけを保存します。
