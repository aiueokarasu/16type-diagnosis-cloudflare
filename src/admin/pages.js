    ...
    body: `
      <main class="auth-wrap">
        <section class="auth-card wide-auth">
          ...
          <h1>初期管理者を2名登録</h1>
          <p class="muted">Cloudflareへ登録した一時セットアップトークンと、2名分の異なる認証情報を入力します。</p>
          <form id="setup-form" class="form-stack two-column">
            <label class="full">セットアップトークン<input name="setupToken" type="password" autocomplete="off" required></label>
            <fieldset><legend>管理者1</legend>
              <label>管理者名<input name="username1" required minlength="3" maxlength="32"></label>
              <label>パスワード<input name="password1" type="password" required minlength="12" maxlength="128"></label>
            </fieldset>
            <fieldset><legend>管理者2</legend>
              <label>管理者名<input name="username2" required minlength="3" maxlength="32"></label>
              <label>パスワード<input name="password2" type="password" required minlength="12" maxlength="128"></label>
            </fieldset>
            <button class="primary-button full" type="submit">2名を登録する</button>
          </form>
          <p id="message" class="message" role="alert"></p>
        </section>
      </main>`,
    ...
    body: `
      ...
      <main class="dashboard">
        ...
        <section class="settings-grid">
          <article class="panel"><p class="eyebrow">ACCOUNTS</p><h2>管理者アカウント</h2><p class="muted small">自分自身は停止できません。必ず1名以上を有効に保ちます。</p><div id="admin-list" class="admin-list"></div></article>
